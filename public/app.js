const TOKEN = window.OSX_CLEAN_TOKEN;
...
async function api(path, options = {}) {
  const res = await fetch(path, {
    ...options,
    headers: { "x-osx-clean-token": TOKEN, "content-type": "application/json", ...options.headers },
  });
...
function startScan() {
  ...
  const params = new URLSearchParams({
    token: TOKEN,
    categories: ids.join(","),
    minItemMB: $("#opt-min").value || String(state.defaults.minItemMB ?? 1),
    downloadsDaysOld: $("#opt-days").value || String(state.defaults.downloadsDaysOld ?? 30),
    recentlyAccessedDays: String(state.defaults.recentlyAccessedDays ?? 7),
    largeFileMinMB: $("#opt-large").value || String(state.defaults.largeFileMinMB ?? 200),
    largeFolderMinMB: $("#opt-folder").value || String(state.defaults.largeFolderMinMB ?? 300),
    duplicatesMinMB: $("#opt-dup").value || String(state.defaults.duplicatesMinMB ?? 5),
    whitelist: "",
  });
...
function fillFileCard(result) {
  ...
  sub.textContent = result.id === "duplicates" ? `${result.count} exact duplicate files` : `${result.count} item${result.count === 1 ? "" : "s"}`;
...
async function init() {
  ...
  $("#opt-min").value = state.defaults.minItemMB ?? "1";
  $("#opt-days").value = state.defaults.downloadsDaysOld ?? "30";
  $("#opt-large").value = state.defaults.largeFileMinMB ?? "200";
  $("#opt-folder").value = state.defaults.largeFolderMinMB ?? "300";
  $("#opt-dup").value = state.defaults.duplicatesMinMB ?? "5";
  startScan();
