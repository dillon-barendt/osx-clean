...
import {lstatSafe, readdirSafe, type Stat} from "../fswalk.js";
import {duSizesParallel} from "../du.js";
import {isWhitelisted} from "../security.js";
import type {ScanContext, ScanItem} from "./types.js";
...
export function toItem(
  ...
  extra?: Partial<ScanItem>,
    ...
    return {id: full, path: full, name: path.basename(full), kind, size, mtime, atime, ...extra};
}
...
export async function listChildren(
  ...
  for (const entry of await readdirSafe(dir)) {
    ...
    const full = path.join(dir, entry.name);
      if (isWhitelisted(full, ctx.options.whitelist)) continue;
...
export async function knownPaths(
  ...
  for (const p of paths) {
      if (isWhitelisted(p, ctx.options.whitelist)) continue;
  ...
  return existing
    .map(({ path: p, st }) =>
        toItem(p, st.kind, sizes.get(p) ?? 0, st.mtime, st.atime, note ? {note} : undefined),
    )
