...
import {type CategoryResult, DEFAULT_OPTIONS, isCommandCategory, type ScanOptions,} from "./scanners/types.js";
import {checkPathAllowed, generateToken, isUserApp, safeEqual, validateForDeletion} from "./security.js";
import {deletePermanently, type ItemResult, moveToTrash} from "./trash.js";
...
function authorized(req: http.IncomingMessage, url: URL): boolean {
  const header = req.headers["x-osx-clean-token"];
...
function parseOptions(params: URLSearchParams): ScanOptions {
  ...
  return {
    ...
    recentlyAccessedDays: num("recentlyAccessedDays", DEFAULT_OPTIONS.recentlyAccessedDays),
...
async function handleOverview(res: http.ServerResponse, req: http.IncomingMessage): Promise<void> {
  ...
  // sized over a small pool of `du` workers and streamed as each completes.
    const homeTask = (async () => {
        ...
        const sizes = await duSizesParallel(homeDirs, controller.signal);
        for (const [dir, size] of sizes) {
            if (size > 1024 * 1024) {
                send("folder", {name: path.basename(dir), path: dir, size});
            }
        }
    })();
...
function revealInFinder(target: string): void {
  if (!checkPathAllowed(target).allowed) return;
