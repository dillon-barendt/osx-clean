...
export function isWhitelisted(p: string, whitelist: string[]): boolean {
  return whitelist.some((pattern) => {
    const value = pattern.trim();
    if (!value) return false;
    if (value.startsWith("/")) return p === value || p.startsWith(value + "/");
...
function canonicalize(rawPath: string): string | null {
...
export function checkPathAllowed(rawPath: string, whitelist: string[] = []): Verdict {
  const p = canonicalize(rawPath);
  if (!p) return { allowed: false, reason: "Path is not absolute." };

  if (isWhitelisted(p, whitelist)) {
    return { allowed: false, reason: "Path is whitelisted (excluded from cleaning)." };
  }
  ...
  for (const prefix of PROTECTED_PREFIXES) {
    if (isUnderRoot(p, prefix)) {
      return { allowed: false, reason: `Protected system area: ${prefix}` };
    }
  }

  const inAllowed = ALLOWED_ROOTS.some((r) => isUnderRoot(p, r));
  if (!inAllowed) {
...
export async function validateForDeletion(
  ...
  const p = canonicalize(rawPath);
  if (!p) return { allowed: false, reason: "Path is not absolute." };
  const textual = checkPathAllowed(p, whitelist);
  ...
  try {
    ...
    if (st.isSymbolicLink()) return { allowed: false, reason: "Symlink targets are not deletable." };
  } catch {
