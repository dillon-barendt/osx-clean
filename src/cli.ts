...
function parseArgs(argv: string[]): { port: number; open: boolean } {
  ...
  for (let i = 0; i < argv.length; i++) {
    ...
    if (arg === "--no-open") open = false;
      ...
      console.log("Usage: osx-clean [--port <n>] [--no-open]");
      process.exit(0);
    }
  }
  return { port, open };
...
function main(): void {
  ...
  server.on("error", (err: NodeJS.ErrnoException) => {
    if (err.code === "EADDRINUSE") {
      console.error(`Port ${port} is in use. Try: osx-clean --port ${port + 1}`);
    } else {
  ...
  server.listen(port, "127.0.0.1", () => {
    ...
    console.log(`  Scan target: this Mac, local-only`);
    console.log(`  Open: ${url}`);
    console.log(`  Tip: use --no-open to keep it headless\n`);
